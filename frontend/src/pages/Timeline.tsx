...
import { MoreHorizontal, Heart, MessageCircle, Edit, Trash2, Send } from "lucide-react";
...
// Chave para salvar comentários no localStorage
...
const Timeline = () => {
    ...
    const [suggestedUserList, setSuggestedUserList] = useState<UserInfo[]>([]);
    const [openCommentSectionId, setOpenCommentSectionId] = useState<number | null>(null);
    const [newCommentText, setNewCommentText] = useState<string>("");
    // Inicializa o estado de comentários buscando do localStorage
    const [localComments, setLocalComments] = useState<LocalCommentsMap>(() => {
        const savedComments = localStorage.getItem(LOCAL_COMMENTS_KEY);
        if (savedComments) {
            try {
                return JSON.parse(savedComments);
            } catch (e) {
                console.error("Falha ao parsear comentários do localStorage", e);
                localStorage.removeItem(LOCAL_COMMENTS_KEY); // Limpa dado inválido
                return {};
            }
        }
    ...
    // Efeito para buscar dados iniciais (não carrega mais comentários daqui)
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Não buscamos comentários da API aqui (ainda)
            } catch (error) { console.error("Erro ao buscar tweets:", error); }
    ...
    // Postar comentário - AJUSTADO para salvar no localStorage
    const handlePostComment = (tweetId: number) => {
        if (!newCommentText.trim()) { toast.error("O comentário não pode ser vazio."); return; }
        if (!userName) { toast.error("Não foi possível identificar o autor do comentário."); return; }

        const newComment: LocalComment = {
            id: `local-${Date.now()}-${Math.random()}`, text: newCommentText.trim(),
            author: userName, timestamp: Date.now()
        };

        // Atualiza o estado
        const updatedCommentsMap = { ...localComments }; // Copia o mapa atual
        const existingComments = updatedCommentsMap[tweetId] || []; // Pega lista atual ou array vazio
        updatedCommentsMap[tweetId] = [...existingComments, newComment]; // Adiciona novo comentário à lista do tweet
        setLocalComments(updatedCommentsMap); // Define o novo mapa no estado

        // <<< SALVAR NO LOCALSTORAGE >>>
        try {
            localStorage.setItem(LOCAL_COMMENTS_KEY, JSON.stringify(updatedCommentsMap));
        } catch (e) {
            console.error("Erro ao salvar comentários no localStorage", e);
            toast.error("Não foi possível salvar o comentário localmente.");
            // Considerar reverter a atualização do estado se o save falhar?
        ...
        setOpenCommentSectionId(null); // Fecha a seção após postar
    };
    ...
    return (
        <div className="min-h-screen flex w-full h-screen overflow-hidden">
            {/* 1. Sidebar Esquerda */}
            <Sidebar {...{ userName, userEmail, seguindo, setSeguindoEstado, abrirModalEdicaoPerfil }} handleLogout={() => { localStorage.clear(); window.location.href = "/"; }} />
            ...
            <main className="flex-grow overflow-y-auto bg-gray-100 border-l border-r border-gray-200">
                <div className="p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {tweets.map((tweet) => {
                            const commentsForThisTweet = localComments[tweet.id] || [];
                            return (
                                <div key={tweet.id} className="bg-white rounded-lg shadow p-4 border border-gray-200">
                                    {/* Cabeçalho e Conteúdo do Tweet */}
                                    <div className="flex justify-between items-start"> <div> <h2 className="font-bold text-gray-900">{tweet.owner?.name || "Usuário Anônimo"}</h2> <p className="text-sm text-gray-500">@{tweet.owner?.email?.split('@')[0] || "username"} · {new Date(tweet.created_at).toLocaleDateString()}</p> </div> <div className="relative"> <button onClick={() => setMenuAberto(prev => prev === tweet.id ? null : tweet.id)} className="text-gray-500 hover:text-gray-700 p-1 rounded-full"> <MoreHorizontal className="w-5 h-5" /> </button> {menuAberto === tweet.id && ( <div className="absolute right-0 mt-1 w-48 bg-white rounded-md shadow-lg border z-10 py-1"> <ul> {userId === tweet.owner?.id && ( <li> <button onClick={() => handleAbrirEditarTweet(tweet)} className="flex items-center space-x-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left"> <Edit className="w-4 h-4"/> <span>Editar</span> </button> </li> )} {userId === tweet.owner?.id && ( <li> <button onClick={() => handleExcluirTweet(tweet.id)} className="flex items-center space-x-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 w-full text-left"> <Trash2 className="w-4 h-4"/> <span>Excluir</span> </button> </li> )} {userId !== tweet.owner?.id && ( <li><button className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left">Denunciar</button></li> )} </ul> </div> )} </div> </div>
                                    <p className="mt-2 text-gray-800 whitespace-pre-wrap">{tweet.content}</p>
                                    {/* Ações do Tweet (Like / Comment) */}
                                    <div className="mt-4 flex items-center space-x-6 text-gray-500">
                                        <button onClick={() => handleToggleComments(tweet.id)} className="flex items-center space-x-1 group hover:text-blue-500">
                                            <div className="p-2 rounded-full group-hover:bg-blue-100"> <MessageCircle className="w-5 h-5" /> </div>
                                            <span>{commentsForThisTweet.length}</span>
                                        </button>
                                        <button onClick={() => handleCurtir(tweet.id)} className={`flex items-center space-x-1 group ${ curtidos[tweet.id] ? "text-red-500" : "hover:text-red-500" }`}> <div className="p-2 rounded-full group-hover:bg-red-100"> <Heart className={`w-5 h-5 ${curtidos[tweet.id] ? 'fill-current' : ''}`} /> </div> <span>{tweet.curtidas ?? 0}</span> </button>
                                    </div>
                                    {/* Seção de Comentários */}
                                    {openCommentSectionId === tweet.id && (
                                        <div className="mt-4 pt-4 border-t border-gray-200">
                                            <div className="flex items-start space-x-3 mb-4">
                                                <textarea value={newCommentText} onChange={handleNewCommentChange} rows={2} placeholder="Escreva seu comentário..." className="flex-grow border border-gray-300 rounded-lg p-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none" />
                                                <button onClick={() => handlePostComment(tweet.id)} disabled={!newCommentText.trim()} className={`p-2 rounded-full text-white transition-colors ${!newCommentText.trim() ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'}`} title="Enviar comentário" > <Send className="w-4 h-4"/> </button>
                                            </div>
                                            <div className="space-y-3">
                                                {commentsForThisTweet.length === 0 && ( <p className="text-xs text-gray-500 text-center">Nenhum comentário ainda.</p> )}
                                                {commentsForThisTweet.sort((a, b) => b.timestamp - a.timestamp).map(comment => (
                                                    <div key={comment.id} className="flex space-x-2 text-xs">
                                                        <div className="bg-gray-100 p-2 rounded-lg flex-grow"> <span className="font-semibold text-gray-800">{comment.author}</span> <p className="text-gray-600">{comment.text}</p> </div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
