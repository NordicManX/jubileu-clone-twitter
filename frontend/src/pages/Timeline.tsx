...
const Timeline = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Carrega dados do usuário do localStorage (assumindo que o Login salvou 'userId')
            const storedEmail = localStorage.getItem("userEmail");
            const storedName = localStorage.getItem("userName");
            const storedId = localStorage.getItem("userId");
            if (storedEmail) setUserEmail(storedEmail);
            if (storedName) setUserName(storedName);
            if (storedId) { const parsedUserId = parseInt(storedId, 10); if (!isNaN(parsedUserId)) {setUserId(parsedUserId); console.log("Timeline mounted, userId set to:", parsedUserId)} else {console.warn("Stored userId is not a valid number:", storedId);}}
    ...
    const handleNovoTweet = async () => {
        ...
        if (!token || userId === null) { toast.error("Login necessário para postar."); return; }
        try {
            const res = await fetch(`${apiUrl}/tweets/`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ content: novoTweet }), });
            ...
            const tweetCriado = await res.json();
             const tweetFormatado = { ...tweetCriado, owner: tweetCriado.owner || { id: userId, name: userName, email: userEmail, created_at: new Date().toISOString() } };
            setTweets([tweetFormatado, ...tweets]); setNovoTweet(""); toast.success("Tweet postado!");
    ...
    const handleSalvarPerfil = async () => {
        ...
        if (!novoNome.trim() || !novoEmail.trim()) { toast.error("Nome e Email não podem ser vazios."); return; }
        try {
            const response = await fetch(`${apiUrl}/api/users/me`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ name: novoNome, email: novoEmail }) });
            if (!response.ok) { const errorData = await response.json().catch(() => ({})); throw new Error(errorData.detail || `Falha ao atualizar perfil (status ${response.status})`); }
            const updatedUserData = await response.json();
            setUserName(updatedUserData.name); setUserEmail(updatedUserData.email);
            localStorage.setItem("userName", updatedUserData.name); localStorage.setItem("userEmail", updatedUserData.email);
            if (userId !== null) {
                setTweets(prevTweets => prevTweets.map(tweet => {
                    if (tweet.owner && tweet.owner.id === userId) { return { ...tweet, owner: { ...(tweet.owner as TweetOwner), name: updatedUserData.name } }; }
                    return tweet; })
                );
            }
            setMostrarModalPerfil(false); toast.success("Perfil atualizado com sucesso!");
        } catch (error) { console.error("Erro ao salvar perfil:", error); toast.error(`Erro ao salvar perfil: ${error instanceof Error ? error.message : String(error)}`); }
    };

    // Excluir Tweet
    const handleExcluirTweet = async (tweetId: number) => {
        // <<< Log de Diagnóstico Adicionado >>>
        console.log(`Tentando EXCLUIR tweet ${tweetId}. ID do usuário no estado: ${userId}`);
        const tweetOwnerId = tweets.find(t => t.id === tweetId)?.owner?.id;
        console.log(`ID do dono do tweet ${tweetId} (frontend): ${tweetOwnerId}`);
        // <<< Fim do Log >>>
        ...
        try {
            const response = await fetch(`${apiUrl}/tweets/${tweetId}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token}` } });
    ...
    const handleAbrirEditarTweet = (tweetParaEditar: Tweet) => {
        // <<< Log de Diagnóstico Adicionado >>>
        console.log(`Tentando ABRIR EDIÇÃO do tweet ${tweetParaEditar.id}. ID do usuário no estado: ${userId}. Dono do tweet (frontend): ${tweetParaEditar.owner?.id}`);
        // <<< Fim do Log >>>

        // A verificação de permissão AGORA é feita aqui E no backend
        if (tweetParaEditar && tweetParaEditar.owner?.id === userId) {
            setEditingTweet(tweetParaEditar);
            setEditedContent(tweetParaEditar.content);
            setShowEditModal(true);
            setMenuAberto(null);
        } else {
            // Mostra erro se o ID não bater, mesmo que o botão tenha aparecido (precaução)
            toast.error("Você não tem permissão para editar este tweet.");
            setMenuAberto(null);
        }
    };
    ...
    const handleSalvarEdicaoTweet = async () => {
        if (!editingTweet || editedContent.trim() === "") return;
         // <<< Log de Diagnóstico Adicionado >>>
         console.log(`Tentando SALVAR edição do tweet ${editingTweet.id}. ID do usuário no estado: ${userId}. Dono do tweet (frontend): ${editingTweet.owner?.id}`);
         // <<< Fim do Log >>>
        const token = localStorage.getItem("token");
        if (!token) { toast.error("Autenticação necessária para editar."); return; }
        // Verificação dupla de permissão no frontend (melhor prática)
        if(editingTweet.owner?.id !== userId) {
             toast.error("Você não tem permissão para editar este tweet.");
             setShowEditModal(false); setEditingTweet(null);
             return;
        }
        try {
            const response = await fetch(`${apiUrl}/tweets/${editingTweet.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ content: editedContent }) });
            if (response.ok) {
    ...
    return (
        <div className="min-h-screen flex w-full h-screen overflow-hidden">
            <Sidebar userName={userName} userEmail={userEmail} seguindo={seguindo} setSeguindo={setSeguindoEstado} setMostrarModal={abrirModalEdicaoPerfil} handleLogout={() => { localStorage.clear(); window.location.href = "/"; }} />
                                     ...
                                     <div className="relative">
                                         ...
                                         {menuAberto === tweet.id && (
                                             <div className="absolute right-0 mt-1 w-48 bg-white rounded-md shadow-lg border z-10 py-1">
                                                 <ul>
                                                     {/* Botões Condicionais */}
                                                     {userId === tweet.owner?.id && ( <li> <button onClick={() => handleAbrirEditarTweet(tweet)} className="flex items-center space-x-2 px-4 py-2 text-sm text-black-700 hover:bg-blue-500 w-full text-left"> <Edit className="w-4 h-4"/> <span>Editar</span> </button> </li> )}
                                                     {userId === tweet.owner?.id && ( <li> <button onClick={() => handleExcluirTweet(tweet.id)} className="flex items-center space-x-2 px-4 py-2 text-sm text-white-600 hover:bg-red-500 w-full text-left"> <Trash2 className="w-4 h-4"/> <span>Excluir</span> </button> </li> )}
