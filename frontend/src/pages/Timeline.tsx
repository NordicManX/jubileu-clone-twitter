...
// Adiciona Menu e X para os botões mobile
import { MoreHorizontal, Heart, MessageCircle, Edit, Trash2, Send, LogOut, Menu, X } from "lucide-react";
import Sidebar from "../components/Sidebar";
import SuggestionsSidebar from "../components/SuggestionsSidebar";
import MobileSidebar from "../components/MobileSidebar"; // <<< Importa o novo componente
...
const Timeline = () => {
    ...
    const [localComments, setLocalComments] = useState<LocalCommentsMap>(() => { /* ... */ const savedComments = localStorage.getItem(LOCAL_COMMENTS_KEY); if (savedComments) { try { return JSON.parse(savedComments); } catch (e) { console.error("Falha ao parsear comentários do localStorage", e); localStorage.removeItem(LOCAL_COMMENTS_KEY); return {}; } } return {}; });

    // --- NOVO Estado para Sidebar Mobile ---
    const [isMobileSidebarOpen, setIsMobileSidebarOpen] = useState(false);

    const navigate = useNavigate(); // Hook para navegação

    // useEffects (como antes)
    useEffect(() => { /* ... fetchData ... */ const fetchData = async () => { try {const res = await fetch(`${apiUrl}/tweets/`);if (!res.ok) throw new Error('Falha ao buscar tweets');const data = await res.json();const tweetsComOwner = data.map((t: any) => ({ ...t, owner: t.owner || null }));setTweets(tweetsComOwner);} catch (error) { console.error("Erro ao buscar tweets:", error); } const storedEmail = localStorage.getItem("userEmail");const storedName = localStorage.getItem("userName");const storedId = localStorage.getItem("userId");if (storedEmail) setUserEmail(storedEmail);if (storedName) setUserName(storedName);if (storedId) { const parsedUserId = parseInt(storedId, 10); if (!isNaN(parsedUserId)) setUserId(parsedUserId); } else { console.warn("Timeline: userId não encontrado no localStorage ao montar."); } const storedFollowStatus = localStorage.getItem('followStatusData'); if (storedFollowStatus) { try { setFollowStatus(JSON.parse(storedFollowStatus)); } catch (e) { console.error("Falha ao parsear follow status do localStorage", e); localStorage.removeItem('followStatusData'); } } setNovoNomeEstado(storedName || ""); setNovoEmailEstado(storedEmail || ""); }; fetchData(); }, []);
    useEffect(() => { /* ... extract users ... */ if (tweets.length > 0 && userId !== null) { const usersMap = new Map<number, UserInfo>(); tweets.forEach(tweet => { if (tweet.owner && tweet.owner.id !== userId && !usersMap.has(tweet.owner.id)) { usersMap.set(tweet.owner.id, { id: tweet.owner.id, name: tweet.owner.name || "Usuário Anônimo" }); } }); setSuggestedUserList(Array.from(usersMap.values())); } else { setSuggestedUserList([]); } }, [tweets, userId]);


    // --- Funções Handler ---
    const handleLogout = () => { toast('Desconectando...'); localStorage.clear(); navigate("/login"); };
    const handleFollowToggle = (targetUserId: number, isCurrentlyFollowing: boolean) => { /* ... */ const newFollowState = !isCurrentlyFollowing; const updatedFollowStatusMap = { ...followStatus, [targetUserId]: newFollowState }; setFollowStatus(updatedFollowStatusMap); try { localStorage.setItem('followStatusData', JSON.stringify(updatedFollowStatusMap)); } catch (e) { console.error("Erro ao salvar follow status no localStorage", e); toast.error("Erro ao salvar preferência de seguir."); } const targetUser = suggestedUserList.find(u => u.id === targetUserId); const targetName = targetUser?.name || `Usuário ${targetUserId}`; if (newFollowState) { toast.success(`Seguindo ${targetName}!`); } else { toast.success(`Deixou de seguir ${targetName}!`); } };
    ...
    // --- JSX ---
    return (
        <div className="min-h-screen flex w-full h-screen overflow-hidden relative"> {/* Adicionado relative */}

             {/* --- Botão Hamburger para Abrir Sidebar Mobile --- */}
             <button
                onClick={() => setIsMobileSidebarOpen(true)}
                className="md:hidden fixed top-3 left-3 z-50 p-2 bg-white rounded-full shadow-md text-blue-500 hover:bg-gray-100"
            ...
            {/* Garanta que Sidebar.tsx tenha 'hidden md:flex' no <aside> */}
            <Sidebar
                userName={userName}
                userEmail={userEmail}
                seguindo={seguindo}
                ...
                // handleLogout não é mais passado para cá
            />
            ...
            <MobileSidebar
                ...
                userName={userName}
                userEmail={userEmail}
                seguindo={seguindo} // Passa o estado seguindo
                setSeguindo={setSeguindoEstado} // Passa a função para alterar
                onEditProfile={abrirModalEdicaoPerfil} // Passa a função para abrir o modal de perfil
            ...
            <main className="flex-grow overflow-y-auto bg-gray-100 border-l border-r border-gray-200">
                {/* ... (Conteúdo da Main como antes) ... */}
                <div className="p-6 pt-16 md:pt-6"> {/* Adiciona padding-top no mobile para não ficar atrás do hamburger */}
                     {/* Área de postagem */}
            ...
            {/* Botão Logout Flutuante (Removido, pois está na MobileSidebar agora) */}
            {/* <button onClick={handleLogout} className="md:hidden fixed ..."> <LogOut /> </button> */}

            {/* Modais (Perfil e Edição de Tweet) */}
